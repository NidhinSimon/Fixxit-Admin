...
const AllService = () => {
  ...
  const confirmDelete = async (i) => {
    try {
      const res = await axios.delete(
        `https://fixxit.shop/admin/services/${itemToDelete._id}`
      );
      console.log(res, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
      if (res.status === 200) {
  ...
  useEffect(() => {
    const services = async () => {
      try {
        const res = await axios.get("https://fixxit.shop/admin/services");
        console.log(res, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.put(
        `https://fixxit.shop/admin/editservice/${editservice._id}`,
        editservice
      );
      console.log(
        editservice,
